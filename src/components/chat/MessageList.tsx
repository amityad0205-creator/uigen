...
import { User, Bot, Loader2, FilePlus, FilePen, FileSearch, Trash2, FolderEdit } from "lucide-react";
...
function describeToolCall(
  ...
  if (toolName === "str_replace_editor" && args) {
    ...
    switch (args.command) {
      case "create":  return { label: `create ${path}`,  Icon: FilePlus };
      case "str_replace":
      case "insert":  return { label: `edit ${path}`,    Icon: FilePen };
      case "view":    return { label: `view ${path}`,    Icon: FileSearch };
    }
  }
  if (toolName === "file_manager" && args) {
    if (args.command === "delete") return { label: `delete ${args.path ?? ""}`, Icon: Trash2 };
    if (args.command === "rename") return { label: `rename ${args.path ?? ""} → ${args.new_path ?? ""}`, Icon: FolderEdit };
  }
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation":
                            ...
                            const { label: toolLabel, Icon: ToolIcon } = describeToolCall(tool.toolName, args);
                            ...
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                {done ? (
                                  <ToolIcon className="w-3.5 h-3.5 text-emerald-600 flex-shrink-0" />
                                ) : (
                                  <Loader2 className="w-3.5 h-3.5 animate-spin text-blue-500 flex-shrink-0" />
                                )}
                                <span className="text-neutral-700 font-mono">{toolLabel}</span>
                              </div>
                            );
                          case "source":
                            return (
